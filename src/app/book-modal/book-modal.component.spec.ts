...
describe('BookModalComponent', () => {
  ...
  it('should display book elements', () => {
    const root = fixture.debugElement.nativeElement;
    expect(root.querySelector('h4').textContent).toContain(booktest.title);
    expect(root.querySelector('.price').textContent).toContain(booktest.price);
    expect(root.querySelector('img').src).toContain(booktest.cover);
    expect(
      root.querySelector('.bookText p:nth-child(3)').textContent
    ).toContain(booktest.synopsis[2]);
  });
});
